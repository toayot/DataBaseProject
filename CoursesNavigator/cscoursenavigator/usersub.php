...
<FONT  COLOR=#000080 SIZE=6><B>&ensp;ข้อมูลการศึกษาของคุณ</B></FONT>

<?php 
$userID = $_SESSION['studentID'];
$add = $con->query("SELECT * FROM subplantest WHERE studentID LIKE '%$userID%' AND gradersemester = '1' AND NOT subStatus = 'p'");

$currentyearchecklist = "SELECT `currentYear` FROM `registerdata` WHERE `studentID` LIKE '%$userID%'";
$currentyearQuerry = mysqli_query($con,$currentyearchecklist);
while($creditchecktemp = mysqli_fetch_assoc($currentyearQuerry)){
    $currentyearcheck = $creditchecktemp['currentYear'];
}
?>
<br><br><br>
<hr>
...
if($currentyearcheck >= 1){
  ...
  <form method="post"  action="grader1.php" >
    <input  type=hidden id="subStatus" name="subStatus" value="s">
    <button type="submit" class="btn btn-link btn-lg">แก้ไขรายวิชา</button></form>';
  }
  else{
    echo ' 
  <form method="post"  action="grader1.php" >
    <input  type=hidden id="subStatus" name="subStatus" value="s">
    <button type="submit" class="btn btn-link btn-lg">เพิ่มวิชาในภาคการศึกษานี้</button></form>';
  }
}
  else{
      echo '</table><br> 
      <p>คุณยังไม่เริ่มเรียนในภาคการศึกษานี้</p>';
  }
?>
<br>
<hr>
...
<?php
$userID = $_SESSION['studentID'];
$add = $con->query("SELECT * FROM subplantest WHERE studentID LIKE '%$userID%' AND gradersemester = '2' AND NOT subStatus = 'p'");

if($currentyearcheck >= 2){
    if (mysqli_num_rows($add) > 0) {
    echo "<table style=\"text-align:center;\">
    <tr>
    <th width='5%'>รหัสวิชา</th>
...
<?php
$userID = $_SESSION['studentID'];
$add = $con->query("SELECT * FROM subplantest WHERE studentID LIKE '%$userID%' AND gradersemester = '3' AND NOT subStatus = 'p'");

if($currentyearcheck >= 3){
    if (mysqli_num_rows($add) > 0) {
    echo "<table style=\"text-align:center;\">
    <tr>
    <th width='5%'>รหัสวิชา</th>
...
<?php
$userID = $_SESSION['studentID'];
$add = $con->query("SELECT * FROM subplantest WHERE studentID LIKE '%$userID%' AND gradersemester = '4' AND NOT subStatus = 'p'");

if($currentyearcheck >= 4){
    if (mysqli_num_rows($add) > 0) {
    echo "<table style=\"text-align:center;\">
    <tr>
    <th width='5%'>รหัสวิชา</th>
...
<?php
$userID = $_SESSION['studentID'];
$add = $con->query("SELECT * FROM subplantest WHERE studentID LIKE '%$userID%' AND gradersemester = '5' AND NOT subStatus = 'p'");

if($currentyearcheck >= 5){
    if (mysqli_num_rows($add) > 0) {
    echo "<table style=\"text-align:center;\">
    <tr>
    <th width='5%'>รหัสวิชา</th>
...
<?php
$userID = $_SESSION['studentID'];
$add = $con->query("SELECT * FROM subplantest WHERE studentID LIKE '%$userID%' AND gradersemester = '6' AND NOT subStatus = 'p'");

if($currentyearcheck >= 6){
    if (mysqli_num_rows($add) > 0) {
    echo "<table style=\"text-align:center;\">
    <tr>
    <th width='5%'>รหัสวิชา</th>
...
<?php
$userID = $_SESSION['studentID'];
$add = $con->query("SELECT * FROM subplantest WHERE studentID LIKE '%$userID%' AND gradersemester = '7' AND NOT subStatus = 'p'");

if($currentyearcheck >= 7){
    if (mysqli_num_rows($add) > 0) {
    echo "<table style=\"text-align:center;\">
    <tr>
    <th width='5%'>รหัสวิชา</th>
...
<?php
$userID = $_SESSION['studentID'];
$add = $con->query("SELECT * FROM subplantest WHERE studentID LIKE '%$userID%' AND gradersemester = '8' AND NOT subStatus = 'p'");

if($currentyearcheck >= 8){
    if (mysqli_num_rows($add) > 0) {
    echo "<table style=\"text-align:center;\">
    <tr>
    <th width='5%'>รหัสวิชา</th>
...
<?php
$userID = $_SESSION['studentID'];
$add = $con->query("SELECT * FROM subplantest WHERE studentID LIKE '%$userID%' AND gradersemester = '9' AND NOT subStatus = 'p'");

if($currentyearcheck >= 9){
    if (mysqli_num_rows($add) > 0) {
    echo "<table style=\"text-align:center;\">
    <tr>
    <th width='5%'>รหัสวิชา</th>
...
<?php
$userID = $_SESSION['studentID'];
$add = $con->query("SELECT * FROM subplantest WHERE studentID LIKE '%$userID%' AND gradersemester = '10' AND NOT subStatus = 'p'");

if($currentyearcheck >= 10){
    if (mysqli_num_rows($add) > 0) {
    echo "<table style=\"text-align:center;\">
    <tr>
    <th width='5%'>รหัสวิชา</th>
...
<?php
$userID = $_SESSION['studentID'];
$add = $con->query("SELECT * FROM subplantest WHERE studentID LIKE '%$userID%' AND gradersemester = '11' AND NOT subStatus = 'p'");

if($currentyearcheck >= 11){
    if (mysqli_num_rows($add) > 0) {
    echo "<table style=\"text-align:center;\">
    <tr>
    <th width='5%'>รหัสวิชา</th>
...
<?php
$userID = $_SESSION['studentID'];
$add = $con->query("SELECT * FROM subplantest WHERE studentID LIKE '%$userID%' AND gradersemester = '12' AND NOT subStatus = 'p'");

if($currentyearcheck >= 12){
    if (mysqli_num_rows($add) > 0) {
    echo "<table style=\"text-align:center;\">
    <tr>
    <th width='5%'>รหัสวิชา</th>
...
<div class="w3-padding w3-display-bottomleft  w3-allerta" style="position: relative;">
