...
if (!$con)
  ...
  if(isset($_POST['subStatus'])){
    ...
    $semesterCheck = $_POST['semesterCheck'];
        ...
        $pickSemester = "SELECT `semesterCheck` FROM `registerdata` WHERE studentID LIKE '$userID'";
        $pickSemtemp = mysqli_query($con,$pickSemester);
        $pickSemRow0 = mysqli_fetch_assoc($pickSemtemp);
        $pickSem = $pickSemRow0['semesterCheck'];
...
<?php
$userID = $_SESSION['studentID'];
$add = $con->query("SELECT * FROM subplantest WHERE studentID LIKE '%$userID%' AND gradersemester = '$semesterCheck'");
...
<div style="margin-left: 15%;margin-top: 2%;float: left;">
    <form action="" method="">
        ...
        <input  type=hidden id="semesterCheck" name="semesterCheck" value="<?php echo "$semesterCheck" ?>">
